...
# quit with q
```
Set relative line numbers in ~/.config/nvim/lua/chadrc.lua
```lua
...
```
Deactivate completion for markdown files in ~/.config/nvim/lua/chadrc.lua
```lua
-- This file needs to have same structure as nvconfig.lua
-- https://github.com/NvChad/ui/blob/v2.5/lua/nvconfig.lua
-- Please read that file to know all available options :(
...
--   pattern = "markdown",
--   callback = function()
--     -- Désactiver la complétion dans les fichiers markdown
--     vim.b.cmp_active = false
--   end,
-- })
vim.api.nvim_create_autocmd("FileType", {
  pattern = "markdown",
